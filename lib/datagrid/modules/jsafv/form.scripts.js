...
function isLogin(s){return (!isShorter(s,LoginLength) && isValid(s.charAt(0),lwr + upr) && isValid(s,lwr + upr + digits + loginchars));};
            ...
            if(((is_required=='r') || ((is_required=='s') && (!isEmpty(field_value)))) ||
                ...
                switch (a_type){
                    ...
                    case 'a': 
                        ...
                        break;                        
                    case 'b':
                        if(!isAlphanumeric(field_value))  { true_value=false; }
                        break;
                    case 't': 
                        switch (b_type){                   
                    ...
                    case 'c':
                            if(is_required=='r'){
                                if(!isChecked(frm,ind))         { true_value=false; }
                            }
                            break;
